...
# 8-Week Implementation Playbook (nested expanders)
with st.expander("📋 8-Week Implementation Playbook", expanded=False):
    st.markdown("**Evidence-Based Consulting Model**: Assessment → Telemetry → Calibration → Optimization → Exit")
    ...
    with st.expander("Week 0: Discovery & Baseline Establishment", expanded=False):
        week0_data = {
            ...
            "Details": [
                ...
                "assessment_baseline.json; champion_profiles.yaml; file_structure_inventory.json; incident_heatmap.json",
                "Self-reported entropy: 68/100; Adoption rate: 22%; Champion coverage: 0% identified",
                "Not deployed",
                "Which 3-5 domains need VTCO first? Initial zone mapping validated?"
            ]
        }
        st.dataframe(pd.DataFrame(week0_data), use_container_width=True, hide_index=True)
        st.info("**Critical Questions**: Last 3 production incidents—which files? Which folder causes panic if junior edits it?")
