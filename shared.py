...
def render_sidebar(use_nav_radio=True, nav_index=0):
    ...
    if use_nav_radio:
        ...
        nav = None
        st.sidebar.markdown("[1. Framework & Architecture](/?nav=1)")
        st.sidebar.markdown("[2. Baseline Assessment (M2)](/?nav=2)")
        st.sidebar.markdown("[3. Scaling Ready (M3)](/?nav=3)")
        st.sidebar.markdown("[4. Implementation Playbook](/?nav=4)")
    ...
    st.sidebar.page_link("pages/7_Strategic_Planning.py", label="Week 1→2 Strategic Planning", icon=":material/insights:")
