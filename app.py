...
nav_index = max(0, min(3, int(nav_param) - 1)) if nav_param.isdigit() else 0
...
elif page == "3. Scaling Ready (M3)":
    runpy.run_path(str(nav_dir / "scaling_ready.py"), run_name="__main__")
elif page == "4. Implementation Playbook":
    runpy.run_path(str(nav_dir / "implementation_playbook.py"), run_name="__main__")
else:
    runpy.run_path(str(nav_dir / "framework_architecture.py"), run_name="__main__")
